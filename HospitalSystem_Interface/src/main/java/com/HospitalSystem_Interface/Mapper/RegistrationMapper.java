...
public interface RegistrationMapper {
    ...
    @Select("select * from v_registration where doctor_id = #{doctor_id} and (patient_name like CONCAT('%', #{keyword}, '%') or patient_spell_code = #{keyword} or (serial_id = #{keyword} and visit_date = DATE_FORMAT(NOW(), '%Y-%m-%d'))) and registration_status = 1 and visit_date < NOW()" +
            "union all " +
